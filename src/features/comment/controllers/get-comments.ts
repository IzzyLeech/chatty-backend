...
export class Get {
  public async comments(req: Request, res: Response): Promise<void> {
    ...
    const cachedComments: ICommentCache[] = await commentCache.getCommentsFromCache(postId);
    const comments: ICommentCache[] = cachedComments.length
      ? cachedComments
  ...
  public async commentsNamesFromCache(req: Request, res: Response): Promise<void> {
    ...
    res.status(HTTP_STATUS.OK).json({ message: 'Post comments', commentsNames });
  }

  public async singleComment(req: Request, res: Response): Promise<void> {
    const { postId, commentId } = req.params;
    const cachedComments: ICommentCache[] = await commentCache.getSingleCommentFromCache(postId, commentId);
    const comments: ICommentCache[] = cachedComments.length
      ? cachedComments
