...
export class Get {
    public async posts(req: Request, res: Response): Promise<void> {
        ...
        let posts: IPost[] = [];
        let totalPosts = 0;
        const cachedPosts: IPost[] = await postCache.getPostsFromCache('post', newSkip, limit);
        ...
        const newSkip: number = skip === 0 ? skip : skip + 1;
        let posts: IPost[] = [];
        const cachedPosts: IPost[] = await postCache.getPostsWithImagesFromCache('post', newSkip, limit);
        ...
        const newSkip: number = skip === 0 ? skip : skip + 1;
        let posts: IPost[] = [];
        const cachedPosts: IPost[] = await postCache.getPostsWithVideoFromCache('post', newSkip, limit);
        posts = cachedPosts.length 
        ? cachedPosts 
