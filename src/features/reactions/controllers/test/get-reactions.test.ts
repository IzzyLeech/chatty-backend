...
describe('Get', () => {
  ...
  describe('reactions', () => {
    it('should send correct json response if reactions exist in cache', async () => {
      ...
      jest.spyOn(ReactionCache.prototype, 'getReactionFromCache').mockResolvedValue([[reactionData], 1]);

      await Get.prototype.reaction(req, res);
      expect(ReactionCache.prototype.getReactionFromCache).toHaveBeenCalledWith(`${postMockData._id}`);
    ...
    it('should send correct json response if reactions exist in database', async () => {
      ...
      const res: Response = reactionMockResponse();
      jest.spyOn(ReactionCache.prototype, 'getReactionFromCache').mockResolvedValue([[], 0]);
