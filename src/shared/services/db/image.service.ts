...
class ImageService {
  ...
  public async removeBackgroundImageFromDB(userId: string): Promise<void> {
    console.log('[removeBackgroundImageFromDB → start]', { userId });

    await UserModel.updateOne({ _id: new mongoose.Types.ObjectId(userId) }, { $set: { bgImageId: '', bgImageVersion: '' } }).exec();
  }
}
